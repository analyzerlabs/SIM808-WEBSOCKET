...
bool WebSocketClient::analyzeRequest() {
    ...
    while (socket_client->is_is_connected() /*&& !socket_client->available()*/) {
        delay(100);
        Serial.println("Waiting...");
    }
    
    //Enviar datos de cabecera
    socket_client->send(cabecera.c_str(), cabecera.length() - 1);
...
bool WebSocketClient::handleStream(String& data, uint8_t *opcode) {
    ...
    if (!socket_client->is_connected() || !socket_client->available())
    {
        return false;
    }      
    ...
    if (length == WS_SIZE16) {
        ...
        length |= timedRead();
        if (!socket_client->is_is_connected()) {
            return false;
        }   
